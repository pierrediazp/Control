...
class ControlGame:
    def __init__(self, runtime):
        ...
        self.startbutton = Button(description='Ejecutar')
        self.startbutton.on_click(self.run)

        self.timeprogress = FloatProgress(value=0, min=0, max=self.runtime, description='Tiempo')
        self.setpoint = FloatSlider(value=0, min=-LIMIT, max=LIMIT, description='Referencia')
        self.controlled = FloatSlider(value=0, min=-LIMIT, max=LIMIT, description='Salida')
        self.controlled.disabled = True
        self.manipulated = FloatSlider(value=0, min=-LIMIT, max=LIMIT, description= 'U(t)')
        self.scoretext = Text(value='0', description='Puntaje')
        self.score = 0
    ...
    def plot(self):
        ...
        F_sp, F_y = axcv.plot(self.ts, self.sps,self.ts, self.cvs)
        axcv.legend((F_sp, F_y), ('SP', 'Y'))
        F_U = axmv.plot(self.ts, self.mvs)
        axmv.legend(F_U,'U')
        plt.show()
